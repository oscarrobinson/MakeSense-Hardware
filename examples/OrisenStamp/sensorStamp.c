...
PROCESS_THREAD(example_abc_process, ev, data)
{
  ...
  //timestamp = 12671189.12; //placeholder for now til time works
  ...
  while(c!='-'){
  	dataLength++;
    c=*((char*)data+dataLength);
  }
  int i=0;
  char broadcast[dataLength];
 ...
 //int brdcstLen = strlen(broadcast);
 int timestampLen = 0;
 ...
 printf("timestampLen: %d timestamp:%lu\n",timestampLen,timestamp);
 char stringToSend[timestampLen+5];
 sprintf(stringToSend,"  %lu X ",timestamp);
 strcat(stringToSend, broadcast);
 printf("%s \n", stringToSend);
 //fflush(stdout);
 ...
 packetbuf_copyfrom(broadcast, dataLength);
 abc_send(&abc);
