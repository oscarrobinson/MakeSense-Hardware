...
/*********************************/
/*** function declaration for the bmp085 drivers ***/
char bmp085_I2C_write(unsigned char device_addr, unsigned char register_addr, unsigned char *register_data, unsigned char write_length){

    uint8_t buffer[] = {register_addr, *register_data};

    i2c_transmitinit( device_addr, write_length + 1, buffer ) ;
    while(!i2c_transferred()) /* Wait for transfer */ ;
    return 0;
}

char bmp085_I2C_read(unsigned char device_addr, unsigned char register_addr, unsigned char *register_data, unsigned char read_length){

    i2c_transmitinit( device_addr, 1, &register_addr ) ;
    while(!i2c_transferred()) /* Wait for transfer */ ;

    i2c_receiveinit( device_addr, read_length, register_data ) ;
    while(!i2c_transferred()) /* Wait for transfer */ ;
    return 0;
}

void bmp085_delay(unsigned int DD){
    clock_delay_msec(DD);
}
...
SENSORS_SENSOR(bmp085_sensor, "bmp085-Sensor", value, configure, status);
