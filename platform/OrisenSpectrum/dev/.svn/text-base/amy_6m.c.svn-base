 ...
 *      Author: mcphillips and Jagun Kwon
...
// Jagun - hopefully this is not going to need
...
int amy_6m_get_bytes_available(void) {

	uint8_t request = AMY_6M_BYTES_AVAILABLE_HIGH ;
	uint8_t read[] = {0,0} ;
	uint16_t receive = 0;
...
//	while(!i2c_transferred()) /* Wait for transfer */ ;
	...
	i2c_receiveinit( AMY_6M_I2C_ADDR, 1, &read[0] ) ;
//	while(!i2c_transferred()) /* Wait for transfer */ ;
	i2c_transferred_timeout(I2C_TIMEOUT);

	request = AMY_6M_BYTES_AVAILABLE_LOW ;
	i2c_transmitinit( AMY_6M_I2C_ADDR, 1, &request ) ;
//	while(!i2c_transferred()) /* Wait for transfer */ ;
	i2c_transferred_timeout(I2C_TIMEOUT);

	i2c_receiveinit( AMY_6M_I2C_ADDR, 1, &read[1] ) ;
//	while(!i2c_transferred()) /* Wait for transfer */ ;
	i2c_transferred_timeout(I2C_TIMEOUT);
...
void amy_6m_get_data_stream(uint8_t *buffer, uint16_t len) {
...
*/
	...
	i2c_receiveinit( AMY_6M_I2C_ADDR, len, buffer ) ;
	while(!i2c_transferred()) ; // Wait for transfer
	//i2c_transferred_timeout(I2C_TIMEOUT);
}
...
int amy_6m_check_ck(uint8_t *buffer, uint8_t len ) {
	uint8_t ck_a = 0;
	uint8_t ck_b = 0;
	uint8_t i = 0;
	...
	//DEBUG - for loop
	PRINTF("DEBUG: amy_6m_check_ck:Buffer contents:\n");
	for (i = 0; i < len; i++) {
			PRINTF("%x ",buffer[i]);
	}
	PRINTF("\n");
...
int amy_6m_nmea_check_ck(uint8_t *buffer, uint8_t len ) {
	char checksum = 0;
	uint8_t ck_a = 0;
	uint8_t ck_b = 0;
...
// This should be used by user instead of amy_6m_get_data_stream
int amy_6m_get_nmea_msg(uint8_t *buffer) {

	uint8_t data[1];
	int len = 0;

	printf ("DEBUG: AVAIL BYTE-I2C: %d\n", amy_6m_get_bytes_available());

	while (1) {
		amy_6m_get_data_stream(data, 1);	// read one byte each time
        ...
        if (gps_msg_received == 1) {
			...
			//if (amy_6m_check_ck(nmea_msg_buf, len)) {
			// Use NMEA-specific checksum (XOR)
			if (amy_6m_nmea_check_ck(nmea_msg_buf, len)) {
				// If checksum is OK, copy msg and return
				strncpy(buffer, nmea_msg_buf, len);
				PRINTF("DEBUG: CHECKSUM PASS!\n");
				return 1;	// Success
			} else {
				PRINTF("DEBUG: CHECKSUM FAIL!\n");
...
void request_RXM_RAW() {
	...
	i2c_transmitinit( AMY_6M_I2C_ADDR, 8, req_RXM_RAW) ;
	while(!i2c_transferred()) ; // Wait for transfer 
}


// This function will store a complete, valid UBX message in gps_ubx
int amy_6m_get_ubx_msg() {
...
/*
	while ((bytes = amy_6m_get_bytes_available()) > 0 &&
		gps_ubx.msg_available == FALSE) {

		amy_6m_get_data_stream(data, 1);// read one byte each time
*/
	while (gps_ubx.msg_available == FALSE) {
		amy_6m_get_data_stream(data, 1);
...
void amy_6m_decode_gps(struct nav_sol_packet *nsp, uint8_t *buffer) {
...
void gps_write(char *buf, int size){
	...
	i2c_transmitinit( AMY_6M_I2C_ADDR, size, buf) ;
	while(!i2c_transferred()) ; // Wait for transfer 
...
/*
	...
	for (i=0; i<size; i++) {
		PRINTF(".");
		//Jagun: FIXME Uart2 is not working as expected.
		//uart2_putc(buf[i]);

		i2c_transmitinit( AMY_6M_I2C_ADDR, 1, &buf[i]) ;
		while(!i2c_transferred())	; // Wait for transfer ;
	}
	PRINTF("\n");
*/
...
//#define UBX_CFG_PRT_BAUDRATE  (u32t)4800
#define UBX_CFG_PRT_BAUDRATE  (u32t)9600
//#define UBX_CFG_PRT_BAUDRATE  (u32t)19200
//#define UBX_CFG_PRT_BAUDRATE  (u32t)38400
...
static void set_ubx_cfg_prt()
{
  ...
  packet[2] = 0x06;
  packet[3] = 0x00;
  packet[4 + 0]  = 0x02;//UART2 //(UBX_CFG_PRT_PS & 0xFF);
...
/*
  ...
  set_ubx_cfg_msg(0xF0, 0x00, 0);  // Enable/Disable NMEA msgs
  set_ubx_cfg_msg(0xF0, 0x01, 0);  // Enable/Disable NMEA msgs
  set_ubx_cfg_msg(0xF0, 0x02, 0);  // Enable/Disable NMEA msgs
  set_ubx_cfg_msg(0xF0, 0x03, 0);  // Enable/Disable NMEA msgs
  set_ubx_cfg_msg(0xF0, 0x04, 0);  // Enable/Disable NMEA msgs
  set_ubx_cfg_msg(0xF0, 0x05, 0);  // Enable/Disable NMEA msgs
  set_ubx_cfg_msg(0xF0, 0x06, 0);  // Enable/Disable NMEA msgs
  set_ubx_cfg_msg(0xF0, 0x07, 0);  // Enable/Disable NMEA msgs
  set_ubx_cfg_msg(0xF0, 0x08, 0);  // Enable/Disable GPZDA
  set_ubx_cfg_msg(0xF0, 0x09, 0);  // Enable/Disable NMEA msgs
  set_ubx_cfg_msg(0xF0, 0x0A, 0);  // Enable/Disable NMEA msgs
  set_ubx_cfg_msg(0xF0, 0x41, 0);  // Enable/Disable NMEA msgs
  set_ubx_cfg_msg(0xF0, 0x42, 0);  // Enable/Disable NMEA msgs
*/
...
//    if (gpsSerial.available()) {
	if (amy_6m_get_bytes_available()) {	// Jagun
		amy_6m_get_data_stream(incoming_char, 1);
