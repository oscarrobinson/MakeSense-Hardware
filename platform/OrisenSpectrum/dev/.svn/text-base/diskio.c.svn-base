...
#define CS_LOW()		spi_ss_low()
#define CS_HIGH()		spi_ss_high()

//#define FCLK_FAST()		SPI->SETUPbits.SCK_FREQ = SPI_FREQ_1500KHZ		// Max Speed for 2G card
#define FCLK_FAST()		SPI->SETUPbits.SCK_FREQ = SPI_FREQ_12MHZ
#define FCLK_SLOW() 	SPI->SETUPbits.SCK_FREQ = SPI_FREQ_375KHZ

#define SPI_8BIT()		{spi_set_sck(8) ; spi_data_len(8) ;}
#define SPI_16BIT()		{spi_set_sck(16) ; spi_data_len(16) ;}
...
extern volatile uint16_t Timer1, Timer2;		/* 1000Hz decrement timer */
...
//#define rcvr_spi_m(p)	spi_set_sck(8); spi_data_len(8) ;SPI->TX_DATAbits.DATA; spi_start(); while(!spi_done()); spi_int_clear(); *(p) = (BYTE)SPI->RX_DATAbits.DATA;

uint8_t xchg_spi (uint8_t dat)
{
	spi_set_sck(8) ;
	spi_data_len(8) ;
	SPI->TX_DATAbits.DATA = dat;
	spi_start() ;
	while(!spi_done()) {}
	spi_int_clear();
	return SPI->RX_DATAbits.DATA ;
}
...
//	while (Timer1) rcvr_spi();

//	printf("**done dummy clocks**\n") ;
